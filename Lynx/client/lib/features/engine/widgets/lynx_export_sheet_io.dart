...
Future<void> _runFullBuild(
  ...
  if (err != null) {
    ...
    if (preset == LynxExportPreset.cart && auth.isAuthenticated) {
      final publish = await showDialog<bool>(
        context: context,
        builder: (ctx) => AlertDialog(
          title: const Text('Аркада'),
          content: const Text('Опубликовать .lynxcart в каталоге аркады?'),
          actions: [
            TextButton(onPressed: () => Navigator.pop(ctx, false), child: const Text('Позже')),
            FilledButton(onPressed: () => Navigator.pop(ctx, true), child: const Text('Опубликовать')),
          ],
        ),
      );
