...
MANIFEST_URL="https://lynx-hub.ru/downloads/engine-manifest.json"
...
if [[ -n "$POSTGRES_PASSWORD" ]]; then
  ...
  if docker ps --format '{{.Names}}' 2>/dev/null | grep -qx waypoint-db; then
    docker exec -i waypoint-db psql -U waypoint -d waypoint -v ON_ERROR_STOP=1 -c \
      "UPDATE nexus_engine_policy SET manifest_url = '${MANIFEST_URL}', recommended_version = '${RECOMMENDED}', updated_at = now() WHERE id = 1;"
  else
...
echo ""
curl -fsS "$MANIFEST_URL" | head -c 120 || echo "  CDN manifest FAIL"
echo ""
ls -la "$DOWNLOADS/engine/"*.lynxengine 2>/dev/null || echo "  (no .lynxengine in $DOWNLOADS/engine yet)"
echo "OK: policy -> $MANIFEST_URL recommended=$RECOMMENDED"
