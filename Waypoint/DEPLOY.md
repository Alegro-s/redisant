# Waypoint: деплой на арендованный сервер (5 минут до первой ценности)
...
Создайте БД и пользователя, задайте `DATABASE_URL` (см. `api/.env.example`). Если поднимаете стек через **Docker Compose** (следующий раздел), отдельно ставить Postgres не нужно.

## 2. Docker Compose (API + Postgres на VPS)

В корне репозитория есть [`docker-compose.yml`](docker-compose.yml): контейнер **PostgreSQL** и сборка **API** из [`api/Dockerfile`](api/Dockerfile). Миграции выполняются при старте бинарника, как и при обычном `cargo run`.

На сервере (Linux с установленным Docker и Compose v2):

1. Склонируйте репозиторий или скопируйте на машину папки `api/` и файлы `docker-compose.yml`, `api/Dockerfile`, `api/.dockerignore`.
...
| Сервер (полная пересборка) | `scripts/vps-build-all.sh` | `git pull`, `docker compose`, сборка `web` в `dist/`; опции `SKIP_*`, `BUILD_FLUTTER=1` — см. заголовок скрипта |
| Сервер | `scripts/vps-git-remote-ssh.sh` | Перевести `origin` на SSH для private repo + Deploy key |
| Сервер / локально | `scripts/vps-minimal-clone.sh` | Только `api/` + compose-файлы (без Flutter) |
...
- второй — порт **Vite** для `web` в этом репозитории (`npm run dev`, см. `web/vite.config.ts`).
