...
pub async fn ingest_handler(
    ...
    let key_record = sqlx::query_as::<_, (Uuid, Uuid)>(
    ...
    let (api_key_id, user_id) = match key_record {
        ...
        Ok(None) => return HttpResponse::Unauthorized().json(ErrorResponse {
    ...
    if let Some(pm) = &state.prometheus {
        pm.ingest_metrics_accepted
            .inc_by(metric_count as u64);
        pm.ingest_logs_accepted.inc_by(log_count as u64);
        pm.ingest_metrics_rejected
            .inc_by(skipped_metrics as u64);
