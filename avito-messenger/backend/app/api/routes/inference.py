...
router = APIRouter(prefix="/api/inference", tags=["inference"], dependencies=[Depends(require_admin_key)])
...
def get_status() -> dict:
    ...
    return {
        ...
        "core_configured": bool(ep.core_base),
        "voice_configured": bool(ep.voice_base),
        "stt_configured": bool(ep.stt_base),
    }
...
def post_chat(body: ChatTestIn) -> dict:
    if not gateway_configured():
        return {"ok": False, "error": "шлюз вывода не настроен"}
